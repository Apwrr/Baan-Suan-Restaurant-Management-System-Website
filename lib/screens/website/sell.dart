...
class _SellState extends State<Sell> {
  ...
  Future<List<Map<String, dynamic>>> fetchTotalSellData() async {
    ...
    final response = await http.get(Uri.parse(
        'https://204rylujk7.execute-api.ap-southeast-2.amazonaws.com/dev/summary/sales-expense-profit/list?year=$selectedYear$queryParameters'));
                          ...
                          if (snapshot.connectionState == ConnectionState.waiting) {
                            ...
                            return PaginatedDataTable(
                              columns: [
                                ...
                                DataColumn(label: Center(child: Text('ยอดขาย'))),
                                DataColumn(label: Center(child: Text('ต้นทุน'))),
                              ],
...
class SalesDataSource extends DataTableSource {
  ...
  DataRow? getRow(int index) {
    ...
    if (sale['summaryDate'] != null) {
      try {
        final date = DateTime.parse(sale['summaryDate']);
    ...
    return DataRow.byIndex(index: index, cells: [
      ...
      DataCell(SizedBox(
        width: 50,
        child: Center(child: Text(sale['totalSales'].toString())),
      )),
      DataCell(SizedBox(
        width: 50,
        child: Center(child: Text(sale['totalExpense'].toString())),
      )),
    ]);
  }
